...
//                      )
  ...
  protected def buildMasterQuantPeptideIonObjectTree( mqPepIon: MasterQuantPeptideIon ): MsiObjectTree = {
   ...
   /**
    * Compule BSC and SSC for each peptide instance of each resultsummary and store information in
  ...
  def computeMasterQuantPeptides( udsMasterQuantChannel: MasterQuantitationChannel,
                                  ...
                                  protSetWeightStructsByProtSetId: Map[Long,ProteinPepsWeightStruct]
...
//	          }
	    	 ...
	    	 peptideSetForPM.getPeptideInstances.foreach(pepInst =>{
	    	   ...
	    	   val qPep = if(quantPepByPepID.contains(pepInst.peptideId)){ quantPepByPepID(pepInst.peptideId)
	    	     }else{
