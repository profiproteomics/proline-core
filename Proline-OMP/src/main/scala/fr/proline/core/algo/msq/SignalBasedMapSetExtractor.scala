...
import fr.proline.core.algo.lcms.BuildMasterMap
...
class SignalBasedMapSetExtractor(
  val lcmsDbCtx: LcMsDbConnectionContext,
  mapSetName: String,
  lcMsRuns: Seq[LcMsRun],
  quantConfig: ILcMsQuantConfig,
  val peptideByRunIdAndScanNumber: LongMap[LongMap[Peptide]]
    ...
    // Map peptides by scan id and scan sequence by run id
      ...
      for (lcmsRun <- lcMsRuns) {
        ...
        for (lcmsScan <- scanSeq.scans) {
          for (peptide <- peptideByRunIdAndScanNumber(lcmsRun.id).get(lcmsScan.initialId)) {
  ...
  private def _rebuildMasterMapUsingPeptides(
    ...
    val tmpRawMap = new RawMap(
      ...
      properties = Some(LcMsMapProperties(ipDeviationUpperBound = Some(ipDevUB.toFloat)))
    )
  ...
  private def _extractFeaturesUsingMs2Events(mzDbFile: File, lcmsRun: LcMsRun): Array[MzDbFeature] = {
    ...
    val peptideByScanNumber = peptideByRunIdAndScanNumber.getOrElse(lcmsRun.id, LongMap.empty[ArrayBuffer[Peptide]])
