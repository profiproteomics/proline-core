...
import fr.proline.core.algo.msi.AbstractResultSetTestCase
...
object WeightedSCCalculatorTest extends AbstractResultSetTestCase with Logging {
...
class WeightedSCCalculatorTest extends Logging {
  
  val executionContext = WeightedSCCalculatorTest.executionContext
  require( executionContext != null, "executionContext is null")
  
  val readRS = WeightedSCCalculatorTest.getRS()
  require( readRS != null, "readRS is null")
...
*/
  ...
  def oneRSMSimpleSC() {
    ...
    val validator = new ResultSetValidator(execContext = executionContext, targetRs = readRS)
    validator.runService()
    
    val rsm = validator.validatedTargetRsm

    val rsmProvider = new SQLResultSummaryProvider(
      msiDbCtx = executionContext.getMSIDbConnectionContext(),
      psDbCtx = executionContext.getPSDbConnectionContext(),
      udsDbCtx = executionContext.getUDSDbConnectionContext()
    )
    ...
    val wsCalculator = new WeightedSCCalculator(
      execContext = executionContext,
      referenceRSM = rsmOp.get,
      rsmToCalculate = Seq(rsmOp.get)
    )
  ...
  def oneRSMIdSimpleSC() {
    ...
    val wsCalculator = new WeightedSCCalculatorWId(
      execContext = executionContext,
      referenceRSMId = rsm.id,
      rsmIdsToCalculate = Seq(rsm.id)
    )
