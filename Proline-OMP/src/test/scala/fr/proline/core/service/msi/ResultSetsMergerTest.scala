...
object ResultSetsMergerTest extends AbstractEmptyDatastoreTestCase with Logging {
...
class ResultSetsMergerTest extends Logging {
  ...
  def testMergeOneRS() {
    ...
    try {
      ...
      val rsMerger = new ResultSetMerger(sqlExecutionContext, Some(Seq(rs1.id,rs1.id)), None )
  ...
  def testMergeTwoRS() {
    ...
    try {
      val rsMerger = new ResultSetMerger(sqlExecutionContext, Some( Seq(rs1.id,rs2.id) ), None )
