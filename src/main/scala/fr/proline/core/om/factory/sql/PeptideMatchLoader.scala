...
import fr.proline.core.om.helper._

class PeptideMatchLoader( val msiDb: DatabaseConfig, val psDb: DatabaseConfig ) {
  
   
  def getPeptideMatches( rsIds: Seq[Int] ): Seq[PeptideMatch] = {
    ...
    // Load peptides
    val uniqPepIds = pmRecords map { _("id").asInstanceOf[Int] } distinct
    ...
    for( pepMatchIdx <- 0 until pmRecords.length ) {
      ...
      if( peptide == null ) throw new Exception("undefined peptide with id ='"+pepId+"'")
      
      // Retrieve the corresponding MS query
      val associatedMsQuery = msQueryById( pepMatchRecord("ms_query_id").asInstanceOf[Int] )
