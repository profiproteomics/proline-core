...
public class V0_6__RS_Hierarchy_Update implements JdbcMigration {
	...
	public void migrate(Connection msiConn) throws Exception {
		...
		PreparedStatement insertRSRelationStmnt = msiConn
			.prepareStatement("INSERT INTO result_set_relation (parent_result_set_id,child_result_set_id) VALUES (? , ?)");
		...
		PreparedStatement testIfRSRelationExistStmnt = msiConn.prepareStatement(
			"SELECT parent_result_set_id, child_result_set_id from result_set_relation WHERE parent_result_set_id = ? AND child_result_set_id = ?");
