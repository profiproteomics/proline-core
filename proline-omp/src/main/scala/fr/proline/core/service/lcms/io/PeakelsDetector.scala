...
//      // FileWriter
//      val file = new File("exportedPeakel.csv")
//      val bw = new BufferedWriter(new FileWriter(file))
//      peakelMatches.foreach( pm => {
//        bw.write(pm.peptide.sequence+"\t"+pm.peptide.readablePtmString+"\t"+pm.peakel.getMz()+"\t"+pm.peakel.getElutionTime()+"\n")
//      })
//      bw.close()
        ...
        try {
          ...
          val missingFtIdAndCoelutingPeakelIds = putativeFts.par.map { putativeFt =>
            ...
            val peakelIds = PeakelDbHelper.findPeakelIdsInRange(
              ...
              mz - halfMzWindow,
              mz + halfMzWindow,
              putativeFt.elutionTime - crossAssignmentConfig.get.ftMappingParams.timeTol,
              putativeFt.elutionTime + crossAssignmentConfig.get.ftMappingParams.timeTol
            )
          ...
          val putativeFtAndMissingPeakelOptTuples = for (putativeFt <- putativeFts.par) yield {
            ...
            Tuple3(putativeFt, mftBuilder, _findUnidentifiedPeakel(
              ...
              putativeFt.mz,
              putativeFt.charge,
              minTime = putativeFt.elutionTime - crossAssignmentConfig.get.ftMappingParams.timeTol,
              avgTime = putativeFt.elutionTime,
              maxTime = putativeFt.elutionTime + crossAssignmentConfig.get.ftMappingParams.timeTol,
  ...
  private def _findUnidentifiedPeakel(
    ...
    */
    ...
    if (filteredPeakels.isEmpty) {
      ...
      //logger.info("Unidentified at mz={} rt={} charge={} matches {} reliables and {} filtered peakel, best is mz={}, rt={}", peakelMz, avgTime, charge, reliablePeakels.size, filteredPeakels.size, nearestPeakelInTime._1.getApexMz(), nearestPeakelInTime._1.getApexElutionTime())
