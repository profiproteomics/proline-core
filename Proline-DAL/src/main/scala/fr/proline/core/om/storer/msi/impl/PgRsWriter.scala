  ...
  override def insertRsProteinMatches(rs: ResultSet, msiDbCtx: DatabaseConnectionContext): Int = {
    ...
    DoJDBCReturningWork.withEzDBC(msiDbCtx, true) { msiEzDBC =>
      ...
      */
      ...
      logger.info(s"Retrieving generated protein match ids...")

      val proteinMatchesByAcc = proteinMatches.filter(_.id < 0).map( pm => pm.accession -> pm).toMap
      ...
      msiEzDBC.select(protMatchUniqueFKQuery, rsId) { r =>
        ...
        proteinMatchesByAcc(accession).id = id
      }
      
      logger.info(s"Linking protein match ids to seq_databases...")
