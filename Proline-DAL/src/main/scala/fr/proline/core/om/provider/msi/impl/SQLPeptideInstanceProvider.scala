...
class SQLPeptideInstanceProvider( val msiDbCtx: DatabaseConnectionContext,                                  
  ...
  private def _getRSMsPepInstRecords( rsmIds: Seq[Int] ): Array[Map[String,Any]] = {
    this.msiSqlExec.selectAllRecordsAsMaps("SELECT * FROM peptide_instance WHERE result_summary_id IN (" + rsmIds.mkString(",") +")")
  }
  
  private def _getPepInstRecords( pepInstIds: Seq[Int] ): Array[Map[String,Any]] = {    
    // TODO: use max nb iterations
    this.msiSqlExec.selectAllRecordsAsMaps("SELECT * FROM peptide_instance WHERE id IN (" + pepInstIds.mkString(",") +")")    
  }
  
  private def _getRSMsPepInstPepMatchMapRecords( rsmIds: Seq[Int] ): Array[Map[String,Any]] = {
    this.msiSqlExec.selectAllRecordsAsMaps("SELECT * FROM peptide_instance_peptide_match_map WHERE result_summary_id IN (" + rsmIds.mkString(",") +")")
  }
  
  private def _getPepInstPepMatchMapRecords( pepInstIds: Seq[Int] ): Array[Map[String,Any]] = {    
    // TODO: use max nb iterations
    this.msiSqlExec.selectAllRecordsAsMaps("SELECT * FROM peptide_instance_peptide_match_map WHERE peptide_instance_id IN (" + pepInstIds.mkString(",") +")")    
  }
