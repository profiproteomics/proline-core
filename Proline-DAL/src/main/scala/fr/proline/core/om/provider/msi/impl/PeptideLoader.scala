...
import scala.collection.mutable.HashMap
  ...
  lazy val ptmDefinitionMap : Map[Int,PtmDefinition] = {
    ...
    // Load PTM records
    psDb.transaction { tx =>       
      tx.selectAndProcess( "SELECT * FROM ptm" ) { r => 
    ...
    psDb.transaction { tx =>
      tx.selectAndProcess( "SELECT * FROM ptm_specificity" ) { r => 
  ...
  def getPeptidePtmRecordsByPepId( peptideIds: Seq[Int] ): Map[Int,Array[Map[String,Any]]] = {
    ...
    peptideIds.grouped(maxNbIters).foreach( tmpPepIds => {      
      ...
      psDb.transaction { tx =>       
        tx.selectAndProcess( "SELECT * FROM peptide_ptm WHERE peptide_id IN (" +
  ...
  def getPeptides( peptideIds: Seq[Int] ): Array[Peptide] = {
    ...
    peptideIds.grouped(maxNbIters).foreach( tmpPepIds => {      
      ...
      // Retrieve peptide PTMs for the current group of peptide ids
      psDb.transaction { tx =>       
        tx.selectAndProcess( "SELECT * FROM peptide WHERE id IN (" +
                   tmpPepIds.mkString(",") + ")" ) { r =>
