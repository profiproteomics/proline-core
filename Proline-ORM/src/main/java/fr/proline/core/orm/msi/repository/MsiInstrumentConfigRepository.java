...
public class MsiInstrumentConfigRepository extends JPARepository {
    ...
    public InstrumentConfig findInstrumConfByNameAndMs1AndMsn(final String name, final String ms1Analyzer,
	...
	if (msnAnalyzer == null) { // Assume NULL <> "" (empty)
	    query = getEntityManager().createNamedQuery("findMsiInstrumConfByNameAndMs1",
		    InstrumentConfig.class);
	} else {
	    query = getEntityManager().createNamedQuery("findMsiInstrumConfByNameAndMs1AndMsn",
		    InstrumentConfig.class);
	    query.setParameter("msnAnalyzer", msnAnalyzer.toLowerCase());
	}

	/* In all cases give InstrumentConfig name and ms1Analyzer */
	...
	if ((instrumConfs != null) && !instrumConfs.isEmpty()) {
		...
		throw new RuntimeException(
			"There are more than one InstrumentConfig for given name, ms1Analyzer and msnAnalyzer");
	    }
