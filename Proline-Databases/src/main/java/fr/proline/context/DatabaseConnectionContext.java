...
public class DatabaseConnectionContext implements Closeable {
	...
	/**
	 * Creates a DatabaseContext instance from an Db connector for JPA
	 * EntityManager use.
	 * 
	 * @param dbConnector
	 ...
	 *            Database DriverType (H2, PostgreSQL, SQLLite).
	 */
	public DatabaseConnectionContext(final Connection connection, final ProlineDatabaseType prolineDatabaseType, final DriverType driverType) {
	 ...
	 * 
	 * @return current EntityManager ; if <code>null</code>, Database is SQL
	 *         JDBC driven.
	 */
	 ...
	 * 
	 * @return <code>true</code> if this context is JPA or mixed JPA / SQL JDBC.
	 *         Returns <code>false</code> if this context is pure SQL.
	 */
	 ...
	 * 
	 * @return current SQL JDBC connection ; if <code>null</code>, Database is
	 *         JPA driven or there is no associated current SQL Connection.
	 */
	...
	public void beginTransaction() throws SQLException {
		...
		synchronized (m_contextLock) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("{} Begin Transaction from DatabaseConnectionContext", getProlineDatabaseTypeString());
			}
	...
	public void commitTransaction() throws SQLException {
		...
		synchronized (m_contextLock) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("{} Commit Transaction from DatabaseConnectionContext", getProlineDatabaseTypeString());
			}
	 ...
	 * 
	 * @param work
	 *            JDBC task to be executed by given <code>EntityManager</code>
	 *            instance, eventually within its <code>EntityTransaction</code>
	 *            .
	 * @param flushEntityManager
	 *            If <code>true</code> and there is no current SQL JDBC
	 *            Connection, flush JPA EntityManager before calling doWork.
	 * @throws SQLException
	 *             the sQL exception
	 */
	public void doWork(final JDBCWork work, final boolean flushEntityManager) throws SQLException {
		...
		synchronized (m_contextLock) {
			...
			if (contextConnection == null) {
				...
				if (LOG.isTraceEnabled()) {
					LOG.trace("{} Executing JDBCWork on JPA EntityManager, flushEntityManager: {}", getProlineDatabaseTypeString(), flushEntityManager);
				}
				...
				final JDBCWork contextWork = new JDBCWork() {
					...
					public void execute(final Connection con) throws SQLException {
						m_connection = con; // Set local SQL JDBC connection in
											// this context
	...
	/**
	 * Executes an SQL JDBC work (returning a result) on raw current SQL
	 * Connection or current <code>EntityManager</code>.
	 * 
	 * @param <T>
	 ...
	 *             the sQL exception
	 */
	public <T> T doReturningWork(final JDBCReturningWork<T> returningWork, final boolean flushEntityManager) throws SQLException {
		...
		synchronized (m_contextLock) {
			...
			if (contextConnection == null) {
				...
				if (LOG.isTraceEnabled()) {
					LOG.trace("{} Executing JDBCReturningWork on raw JDBC Connection", getProlineDatabaseTypeString());
				}
				...
				if (m_connection != null && m_closableConnection) {
					...
					try {
						...
						if ((currentTransaction != null) && currentTransaction.isActive()) {
							LOG.info("{} Rollback EntityTransaction from DatabaseConnectionContext.doClose()", prolineDbType);

							try {
								currentTransaction.rollback();
							} catch (Exception ex) {
								LOG.error("Error rollbacking DatabaseConnectionContext EntityTransaction for " + prolineDbType, ex);
							}
