...
CREATE TABLE public.run (
                ...
                CONSTRAINT run_pk PRIMARY KEY (raw_file_name)
);
...
COMMENT ON COLUMN public.project.owner_id IS 'The owner of this project. The owner is also a member of the project and then is represented in ''''project_user_map''''';
...
CREATE TABLE public.project_user_map (
                ...
                serialized_properties TEXT,
                CONSTRAINT project_user_map_pk PRIMARY KEY (project_id, user_account_id)
);
COMMENT ON TABLE public.project_user_map IS 'Mappinng table between user_account and project table';
COMMENT ON COLUMN public.project_user_map.serialized_properties IS 'A JSON string which stores optional properties (see corresponding JSON schema for more details).';
...
ALTER TABLE public.project_user_map ADD CONSTRAINT user_account_project_user_map_fk
...
ALTER TABLE public.run ADD CONSTRAINT user_account_run_fk
FOREIGN KEY (owner_id)
REFERENCES public.user_account (id)
ON DELETE NO ACTION
ON UPDATE NO ACTION
...
REFERENCES public.run (raw_file_name)
ON DELETE NO ACTION
ON UPDATE NO ACTION
NOT DEFERRABLE;
...
ALTER TABLE public.project_user_map ADD CONSTRAINT project_project_user_map_fk
FOREIGN KEY (project_id)
REFERENCES public.project (id)
ON DELETE NO ACTION
ON UPDATE NO ACTION
